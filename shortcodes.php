...
function steemtemplates_pdflink($attr, $content) {
    if ($attr['href']) {
...
add_shortcode('pdf', 'steemtemplates_pdflink');

function steemtemplates_related_posts_shortcode( $atts ) {
...
add_shortcode('related_posts', 'steemtemplates_related_posts_shortcode');

function steemtemplates_showads($atts) {
    extract(shortcode_atts(array(
        'client' => '',
        'slot' => '',
        'width' => '250',
...
add_shortcode('adsense', 'steemtemplates_showads');

function steemtemplates_chart_shortcode( $atts ) {
    extract(shortcode_atts(array(
        'data' => '',
        'colors' => '',
        'size' => '400x200',
...
add_shortcode('chart', 'steemtemplates_chart_shortcode');

function steemtemplates_tweetmeme(){
...
add_shortcode('tweet', 'steemtemplates_tweetmeme');
/*map.js will enqueue here if the shortcode will be applied*/
function steemtemplates_googleMapShortcode($atts, $content = null)
{
    wp_enqueue_script('tt_googlemaps');
    $marker_text = steemtemplates_theme_option('ttr_marker_text');
    ...
    $mapType = steemtemplates_theme_option('ttr_googlemap_type');
    $latitude =  steemtemplates_theme_option('ttr_map_latitude');
    $longitude = steemtemplates_theme_option('ttr_map_longitude');
    $width = steemtemplates_theme_option('ttr_map_width');
    $height = steemtemplates_theme_option('ttr_map_height');
    ...
    if(steemtemplates_theme_option('ttr_marker_enable')):
        echo'
...
add_shortcode("googlemap", "steemtemplates_googleMapShortcode");


function steemtemplates_cwc_youtube($atts) {
    extract(shortcode_atts(array(
        "value" => 'http://',
        "width" => '475',
        "height" => '350',
...
add_shortcode("youtube", "steemtemplates_cwc_youtube");

function steemtemplates_cwc_member_check_shortcode( $atts, $content = null ) {
    if ( is_user_logged_in() && !is_null( $content ) && !is_feed() )
        return $content;
    return '';
}

add_shortcode( 'member', 'steemtemplates_cwc_member_check_shortcode' );

function steemtemplates_donate_shortcode( $atts ) {
    extract(shortcode_atts(array(
...
add_shortcode('donate', 'steemtemplates_donate_shortcode');

function steemtemplates_contact_form($atts)
{
    ob_start();
	 extract( shortcode_atts( array(
    ...
    <form method="post" class="form-horizontal" id="ContactForm0" style="<?php echo $styles;?>">
      ...
      <div class="form-group">
        <label class="col-sm-4 control-label required">
          <?php echo(__('Email:',"BlockTradesAffiliatesV1"));?>
        </label>
      ...
      <div class="form-group">
        <label class="col-sm-4 control-label">
          <?php echo(__('Message:',"BlockTradesAffiliatesV1"));?>
        </label>
      ...
      <div class="form-group">
        <div class="col-sm-8 col-sm-offset-4">
          <input type="submit" value="<?php _e($btntitle , '','BlockTradesAffiliatesV1');?>" name="submit_values" class="<?php echo $btnclasses;?>">
        </div>
      </div>
...
<?php
  ...
  if (!function_exists('steemtemplates_contact_form_generate_response'))
{
    function steemtemplates_contact_form_generate_response($type, $message){
    ...
    {     	
        ...
        $message .= __('Message:   ',"BlockTradesAffiliatesV1").$_POST['message_text'].' '.$sender;
        ...
        if(empty($nameErr) && empty($emailErr))
        {
	        if($captcha_enable){
		        if($captcha_res){
			        ...
			        steemtemplates_contact_form_generate_response("success", $message_sent ); //message sent!
		        }
		        else{
		       		steemtemplates_contact_form_generate_response("error", "Robot verification failed, please try again.");
		        }
	        }
	        else{
		        wp_mail($to, $subject, $message,$headers);
        ...
        {
        	if(!empty($nameErr)){
				 steemtemplates_contact_form_generate_response("error", $nameErr); //message wasn't sent
			}
        	elseif(!empty($emailErr)){
				steemtemplates_contact_form_generate_response("error", $emailErr); //message wasn't sent
			}			       
        }
    }
    return ob_get_clean();
...
add_shortcode('contact_us_form', 'steemtemplates_contact_form');
